  ...
  socket.on('addUser', (data) => {
    if (data.data.createdDate) {
      ...
      if (moment(month + '-' + day + '-' + year ).diff(moment(), 'days') >= -12) {
        $('.new .names').prepend("<div data-fbid=" + data.data.fbID + "><img class='new-user-icon' src='/imgs/star-icon.svg' alt='New User'><img src='" + data.data.photo + "' alt='profile photo'><p>" + data.data.fullName + "</p></div>")
      }
    } else {
